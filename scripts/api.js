...
const transformDataForSwagger2 = function (data) {
    ...
    for (const path in data.paths) {
        ...
        for (const method in item) {
            ...
            item[method]['Content-Type'] = item[method].consumes ? item[method].consumes[0] : ""
            apisArr.push({
                path,
                method,
                ...item[method]
...
const buildOne = function (data) {
    // 处理url数据
    // 待生成的接口
    let urlArr = utils.cleanEmptyInArray(data.path.split("/"));
    // 文件生成目录
    ...
    const API_URL = `'${utils.cleanEmptyInArray(data.path.split("/")).join("/")}'`;
