...
import {uploadAttrTranslate} from '@/services/product1'
/**
/*  
/*
/*
*/
const UploadAttr = React.forwardRef((props, ref) => {
    const [loading, setLoading] = useState(false)
    const [params, setParams] = useState({})
    const [uploadAttrStr,setUploadAttr] =useState('')
    const [tempUrl,setTempUrl] = useState('https://file.fingo.shop/upload/others/2020-02/06/18298469759992971395072.xlsx')
    const handleOk = () => {
        console.log(uploadAttrStr)
        if(uploadAttrStr==''){
            props.cancel()
        }else{
            let params={
                uploadUrl:uploadAttrStr
            }
            uploadAttrTranslate(params).then(resultes=>{
        ...
        <div>
            ...
            >
                ...
                >
                    ...
                    {``}
                    <div>
                    通过上传表格，可批量修改未被商品使用的属性名称； 请<a onClick={uploadDetail}>下载模版</a>并按照模板中的指导说明制作表格{uploadAttrStr}
                    </div>
                </Upload>
            </Modal>
        </div>
